<template>
    <header class="w-full flex justify-center border-b-2">
            <LayoutContainer>
                <div id="header-wrapper" class="flex items-center justify-between py-2 gap-5 px-3">
                    <SiteLogo />
                    ...
                    <div v-if="isAuthenticated">
                        <div class="bg-gray-100  flex items-center justify-center">
                            ...
                            <div class="relative inline-block text-left">
                                <button  @click="isOpen = !isOpen" id="dropdown-button" dropdown-area class="inline-flex justify-center w-full px-2 py-2 text-sm font-semibold text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-100 focus:ring-blue-500">
                                    <svg id="dropdown-x" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                        <path id="dropdown-x" stroke-linecap="round" stroke-linejoin="round" d="M3.75 5.25h16.5m-16.5 4.5h16.5m-16.5 4.5h16.5m-16.5 4.5h16.5" />
                                    </svg>
                                </button>
                                <div v-if="isOpen" id="dropdown-menu" dropdown-area class="origin-top-right absolute right-0 mt-2 w-full md:w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5">
...
<script setup>
    ...
    document.addEventListener('click',(event) => {
        if (!(event.target.id === 'dropdown-button' || event.target.id === 'dropdown-menu' || event.target.id === 'dropdown-x')) {
            isOpen.value = false;
        }
    })
