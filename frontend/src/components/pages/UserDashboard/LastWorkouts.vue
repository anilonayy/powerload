<template>
  <Panel class="md:p-4">
    ...
    <div v-if="loaded">
          ...
          <div class="p-2 md:p-4 pb-1 bg-white border rounded-xl text-gray-800 text-xs relative">
            <div class="flex justify-between">
              <div class="text-gray-400 text-xs">
                {{ $t('DASHBOARD.LAST_WORKOUTS.TIME') }}:
            ...
            <div class="flex gap-2">
              <router-link
                :to="{ name: 'show-workout-history', params: { workoutLogId: data.id } }"
                class="font-semibold hover:text-yellow-800 hover:underline cursor-pointer flex gap-1 items-center"
              >
                {{ data?.workout?.name }}
      ...
      <div v-else class="text-sm text-gray-600 italic mt-4">Henüz antrenman geçmişiniz bulunmamaktadır.</div>
    </div>
