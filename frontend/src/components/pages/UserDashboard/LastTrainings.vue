<template>
    <Panel class="md:p-4">
        <div id="card-header" class="border-b border-1 mb-2 pb-2 relative">
            <h2 class="text-2xl font-bold">{{ $t('DASHBOARD.LAST_TRAININGS.TITLE') }}</h2>
            
            <div class="text-xs underline">
                <router-link :to="{ name: 'training-history' }">{{ $t('DASHBOARD.LAST_TRAININGS.SEE_ALL') }}</router-link>
            </div>
        </div>
                ...
                <div v-for="(data, index) in list" :key="index">
                    <div class="p-4 pb-1 bg-white border rounded-xl text-gray-800  text-xs">
                        <div class="flex justify-between">
                            <div class="text-gray-400 text-xs">{{ $t('DASHBOARD.LAST_TRAININGS.TIME') }}: {{ data?.duration }}</div>
