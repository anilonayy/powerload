  ...
  <div>
    <div class="mx-3 relative">
      <div class="h-6 w-full flex justify-between items-center mt-10 relative">
        <div
          class="previous-exercise absolute left-0 cursor-pointer"
          v-if="pageIndex !== 0"
      ...
      <h3 v-if="pageIndex === 1 || pageIndex === 0" class="font-bold text-center my-4">
        {{ headerTitle }}
      </h3>
      ...
      <div v-if="pageIndex === 0">
        ...
        <SelectWorkout :workouts="workouts" @select-workout="selectWorkout($event)" v-else />
      </div>

      <div v-else-if="pageIndex === 1" class="flex flex-col gap-3">
        <SelectWorkoutDay :selectedWorkout="selectedWorkout" @select-workout-day="selectWorkoutDay($event)" />
      </div>
    ...
    <div
      v-if="!(pageIndex === maxIndex && maxIndex >= 2) && pageIndex !== 0"
      @click="giveUp()"
...
onMounted(async () => {
  ...
  if ((workouts.value[0] || {}).id === 0 || workouts.value.length === 0) {
    ...
    if (lastWorkout?.data?.exercises?.length) {
      (
        (workoutData.find((workout) => workout.isSelected).days.find((day) => day.isSelected) || {}).exercises || []
      ).map((exercise) => {
        const onWorkoutData = [];
