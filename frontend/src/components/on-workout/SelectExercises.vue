        ...
        <div class="sets">
          <div class="flex flex-col">
            <div v-if="data?.isPassed">
              <div
                class="w-full p-4 rounded-xl bg-yellow-200 text-yellow-700 text-sm border border-1 border-yellow-500"
              >
            ...
            <div v-if="data.onWorkout?.length">
              <div
                v-for="(trainSet, index) in data.onWorkout"
                :key="index"
                class="py-10"
                :class="{ 'border-b border-gray-300': data?.onWorkout?.length - 1 !== index }"
              >
                <div class="flex gap-3 items-end font-bold">
                  <div class="whitespace-nowrap text-7xl" style="line-height: 0.8">{{ index + 1 }}.</div>
                  <!-- <div>
